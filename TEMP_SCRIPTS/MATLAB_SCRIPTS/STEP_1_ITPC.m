%% Time Frequency
% 1) Calculate ITC on several blocks of trials


%% NOTE: this script was prepared to be launched on already pre-processed data from tDCS-ASSR
% thr
...
working_folder = '/storages/LDATA/Code/Scripts_ASSR_Plasticity';
...
% this is to split time frequency in non-overlapping blocks
blocks = {[1:30], [31:60], [61:90], [91:120], [121:150], [151:180]}
blocks_names= {'block1', 'block2', 'block3', 'block4', 'block5', 'block6'}
...
for iSubj = 1:length(Subj_cond)
	...
	% important 
	if iBlock==6
		curr_files=Subj_cond{iSubj}{iCond}{151:end};
        
        else
        curr_files=Subj_cond{iSubj}{iCond}{blocks{iBlock}};
	end
        ...
        Res = bst_process('CallProcess', 'process_add_tag', Res, [], ...
            'tag',    ['ITPC', '_', cond_names{iCond},'_',  blocks_names{iBlock}], ...
            'output', 2);  % Add to file name

        % Process: Add tag:
        Res = bst_process('CallProcess', 'process_add_tag', Res, [], ...
            'tag',    ['ITPC| ', cond_names{iCond}, '| ', blocks_names{iBlock}], ...
