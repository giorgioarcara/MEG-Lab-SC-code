...
my_sFiles = sel_files_bst({my_sFiles_ini.FileName}, 'hilbert');
my_sFiles = sel_files_bst(my_sFiles, 'MAP001');
my_sFiles = sel_files_bst(my_sFiles, 'Response_adj');
...
tasks_subj_runs = {4:6}
tasks_subj_runs_name={'wordrep_resp'}
...
for iSubj = 1:length(Subj_grouped)
    ...
    Res = bst_process('CallProcess', 'process_add_tag', Res, [], ...
        'tag',    [SubjectNames{iSubj}, '| ',tasks_subj_runs_name{iTask}] , ...
        'output', 1);  % Add to comment
    
    % Process: Add tag: SubjectName
    Res = bst_process('CallProcess', 'process_add_tag', Res, [], ...
        'tag',     [SubjectNames{iSubj}, '_',tasks_subj_runs_name{iTask}], ...
