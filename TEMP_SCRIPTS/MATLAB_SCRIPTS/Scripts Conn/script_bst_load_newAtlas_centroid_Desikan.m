...
for iSubj=2:length(my_subjects.Subject);
    ...
    iSurface=find(~cellfun(@isempty, regexpi({my_subjects.Subject(iSubj).Surface.FileName}, 'tess_cortex_mid_low.mat')));
    ...
    original_atlas=find(~cellfun(@isempty, regexpi({mycortex.Atlas.Name}, 'Desikan-Killiany')));
